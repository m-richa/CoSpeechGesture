...
# files = glob.glob(f'./data/{idx}-SHOW1/jo1_fullbody_rot180/*.obj')
...
device = 'cuda'
T = np.load(f'./data/{idx}-SHOW1/all_transl_SHOW_{idx}.npy',)[::sampling_rate][:n_train]
...
res = 256
save_path = f'./checkpoints/show-{idx}-upper-hands-uv-texture-fitting-nogloss-{res}'

os.makedirs(save_path, exist_ok=True)
save_path_images = f'{save_path}/images'
os.makedirs(save_path_images, exist_ok=True)

load_path = f'./data/{idx}-SHOW1/'
meshes = torch.load(os.path.join(load_path, 'meshes_train.pt'))[::sampling_rate][:n_train]
print(len(meshes))
# meshes = []
...
files = glob.glob(f'./data/{idx}-SHOW1/matted/*')
files.sort()
files = files[::sampling_rate]
target_rgb = []
for i in tqdm(range(n_train)):
...
# The optimizer
optimizer = torch.optim.SGD([texture_image], lr=10.0, momentum=0.9)
...
for i in loop:
    ...
    if i % plot_period == 0:
        ...
        with torch.no_grad():
            ...
            torch.save(texture_image.detach().cpu(), os.path.join(save_path, 'texture_uv.pt'))
    # Optimization step
    sum_loss.backward()
    optimizer.step()
