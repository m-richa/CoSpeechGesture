...
class CustomBodyDataset(BaseDataset):
    def __init__(self, opt):
        ...
        # gt_feature_path = glob.glob(self.root+'/body_mesh_vit/*.png')
        gt_feature_path = glob.glob(self.root+'/body_mesh_openpose/*.png')
        # gt_img_path = [path for path in gt_img_path if '[No_Face]' not in path]
        # gt_img_path.sort()
        gt_feature_path.sort()
        ...
        # gt_feature_path = [path.replace('origin', 'body_mesh') for path in gt_img_path]
        # gt_img_path = [path.replace('body_mesh_vit', 'origin') for path in gt_feature_path]
        gt_img_path = [path.replace('body_mesh_openpose', 'origin') for path in gt_feature_path]
    ...
    def __getitem__(self, ind):
        ...
        # print(feature_map.shape, gt_image.shape)
        return_list = {'feature_map': feature_map, 'cand_image': cand_image, 'tgt_image': gt_image, 'weight_mask': gt_image, 'cand_feature': gt_image, 'mask': mask}
