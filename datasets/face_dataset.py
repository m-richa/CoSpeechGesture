    ...
    def __init__(self, opt):
        ...
        if self.opt.isTrain:
            for i in range(self.clip_nums):
                ...
                example = imread(os.path.join(clip_root, 'example.png'))
                ...
                if self.opt.isH5:
                    ...
                    tgt_paths = glob.glob(self.root+'/training_imgs/*.png')
                    tgt_paths.sort()
        ...
        # test mode        
            ...
            #landmarks
            landmark_path = os.path.join(self.root, 'tracked2D_normalized_pts_fix_contour.npy')
            self.landmarks2D[0] = np.load(landmark_path).astype(np.float32)

            # shoulder
            shoulder_path = os.path.join(self.root, 'normalized_shoulder_points.npy')
            self.shoulders[0] = np.load(shoulder_path)
            shoulder3D_path = os.path.join(self.root, 'shoulder_points3D.npy')
            self.shoulder3D[0] = np.load(shoulder3D_path)

            change_paras = np.load(os.path.join(self.root, 'change_paras.npz'))
            scale, xc, yc = change_paras['scale'], change_paras['xc'], change_paras['yc']
            x_min, x_max, y_min, y_max = xc-256, xc+256, yc-256, yc+256
            x_min, x_max, y_min, y_max, self.image_pad[0] = max(x_min, 0), min(x_max, w), max(y_min, 0), min(y_max, h), None
            ...
            for j in range(4):
                try:
                    output = imread(os.path.join(self.root, 'candidates', f'normalized_full_{j}.jpg'))
                except:
                    imgc = imread(os.path.join(self.root, 'candidates', f'full_{j}.jpg'))
                    output = self.common_dataset_transform(imgc, i)
                    imsave(os.path.join(clip_root, 'candidates', f'normalized_full_{j}.jpg'), output)
                output = AP.transforms.ToTensor(normalize={'mean':(0.5,0.5,0.5), 'std':(0.5,0.5,0.5)})(image=output)['image']
                tmp.append(output)
            self.full_cand[0] = torch.cat(tmp)
            ...
            if self.opt.isH5:
                ...
                tgt_paths.sort()
                image_length = len(tgt_paths)
                self.tgts_paths[0] = tgt_paths
            
    
    def __getitem__(self, ind):
        # print('ind', ind)
        # print('sample_start', self.sample_start)
        ...
        #transform
        self.image_transforms[0] = A.Compose([A.Resize(np.int32(self.opt.loadSize), np.int32(self.opt.loadSize))])
        ...
        if self.opt.isH5:
            ...
            # do transform
            tgt_image = self.common_dataset_transform(tgt_image, dataset_index)
        else:
            #load from folder
            # try:
            tgt_file_path = self.tgts_paths[dataset_index][target_ind]
            tgt_image = imread(tgt_file_path)

            #     # do transform
            tgt_image = self.common_dataset_transform(tgt_image, dataset_index)
            # except:
            # tgt_image = np.zeros(((512, 512, 3))).astype(np.uint8)
        ...
        feature_map = feature_map * 0.5020
...
#        plt.imshow(mouth_mask[:,:,0])
    ...
    def get_transform(self, dataset_name, keypoints=False, n_img=1, n_keypoint=1, normalize=True, flip=False):
        ...
        transform = A.Compose([
                ...
                AP.transforms.ToTensor(normalize={'mean':(0.5,0.5,0.5), 'std':(0.5,0.5,0.5)} if normalize==True else None)],
    ...
    def __len__(self):  
        if self.opt.isTrain:
            ...
            # return 1
            return self.landmarks2D[0].shape[0]
